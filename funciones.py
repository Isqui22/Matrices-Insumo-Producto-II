...
def inversaLU(L, U, P=None):
    ...
    # Resolver L * Y = P * I
    Y = scipy.linalg.solve_triangular(L, np.dot(P, Id), lower=True)
...
"""
en inversaLU están usando la función solve_triangular(A,B) donde A y B son dos matrices de nxn, en lugar de solve_triangular(A,v) donde A es de nxn y v es de nx1. En el contexto de este TP sólo está permitido usar solve_triangular como en el segundo caso, ya que pueden estar calculando inversas sin entender cuál es el procedimiento subyacente.
"""
...
def obtener_delta_p_modelo_simple(Arr,delta_dr):
    ...
    Leontief = Irr-Arr
    Inversa_Leontief = obtenerInversa(Leontief)
    return Inversa_Leontief@delta_dr
...
def obtener_delta_p_formula_completa(Arr,Ass,Ars,Asr,delta_dr):
    ...
    Leontief_ss = Iss-Ass
    Inversa_Leontief_ss = obtenerInversa(Leontief_ss)
    Leontief_modificado = Irr - Arr - Ars@(Inversa_Leontief_ss@Asr)
    Inversa_Leontief_modificado = obtenerInversa(Leontief_modificado)
    return Inversa_Leontief_modificado@delta_dr

def obtener_demanda_sector_r(Arr,Ars,pr,ps):
    """
    Recibe:
        Arr (shape (n,n)): matriz de coeficientes técnicos de la economía intraregional de la región r
    ...
    Devuelve:
        demanda del sector r
    """
    Irr = np.eye(Arr.shape[0])
    Leontief = Irr-Arr
    return Leontief@pr - Ars@ps
